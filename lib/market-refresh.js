...
export async function runMarketRefresh(entries, redis) {
  ...
  if (uaImageLots.length > 0) {
    try {
      ...
      // HMGET only the lots we have images for — HGETALL on the full catalog
      // exceeds Upstash's 10 MB response limit.
      const catalog = await redis.hmget('wh:ua:catalog', ...imgMap.keys()) ?? {}
      const patches = {}
