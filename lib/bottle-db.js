...
// Higher wins on field conflicts. User corrections are authoritative — they sit
// ABOVE automated sources so a hand-fixed MSRP/distillery is never silently
// reverted by the next AI-vision or UPC-API scan (the old map ranked user lowest
// at 35, below ai 50 and upc-api 45 — an inversion that wiped user fixes).
const CONFIDENCE = {
  user:     90,
  algolia:  70,
  ua:       70,
...
function similarity(a, b) {
  ...
  // Every number in the query must appear in the candidate.
  // The candidate may have extra numbers (e.g. vintage year) — that's fine.
  const aNums = new Set(aTokens.filter(t => /^\d+$/.test(t)))
  const bNums = new Set(bTokens.filter(t => /^\d+$/.test(t)))
  for (const n of aNums) if (!bNums.has(n)) return 0
...
function mergeRecord(existing, incoming, source) {
  ...
  // UPCs are append-only — never overwrite, always union
  if ('upcs' in incoming && Array.isArray(incoming.upcs)) {
    const existingSet = new Set(existing.upcs ?? [])
    for (const u of incoming.upcs) if (u) existingSet.add(u)
    merged.upcs = [...existingSet]
  }

  for (const [key, value] of Object.entries(incoming)) {
    if (key.startsWith('_') || key === 'slug' || key === 'upcs') continue
    ...
    const existingSrc  = fieldMeta[key]?.source
    const existingVal  = existing[key]

    // A user correction is sacrosanct — never clobbered by an automated source,
    // regardless of stored confidence. This also protects legacy user fields
    // that were written under the old (lower) user confidence.
    if (existingSrc === 'user' && source !== 'user' && existingVal != null) continue

    // Write if: field is empty, or incoming has strictly higher confidence.
    // Strict `>` (was `>=`) stops equal-confidence re-runs from churning values.
