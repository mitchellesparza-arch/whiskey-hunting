 ...
 */
async function resolveSlug(redis, { name, upc, algoliaObjectId }) {
  // 1. Exact slug
  ...
  // 5. Fuzzy scan
  // Only fires when the four cheap checks above all miss — typically only for
  // brand-new bottles or heavily abbreviated name variants.
  ...
  for (const s of allSlugs) {
    ...
    // Check incoming name against canonical name + all stored aliases
 ...
 */
export async function upsertBottle(incoming, source = 'user') {
