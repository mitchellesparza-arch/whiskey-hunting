...
/**
 * Notify the app owner when a brand-new user signs up.
 *
 * @param {string} name   — display name from Google
 * @param {string} email  — lower-cased email address
 */
export async function sendNewUserEmail(name, email) {
  const to = process.env.ALERT_EMAIL
  if (!to) {
    console.warn('[email] ALERT_EMAIL not set — skipping new-user email')
    return
  }

  const appUrl   = process.env.NEXT_PUBLIC_BASE_URL ?? 'https://whiskey-hunter.vercel.app'
  const adminUrl = `${appUrl}/admin`
  const subject  = `👋 New member: ${name}`
  const joinedAt = new Date().toLocaleString('en-US', { dateStyle: 'full', timeStyle: 'short' })
...
<body style="margin:0;padding:0;background:#0f0a05;color:#f5e6cc;font-family:'Segoe UI',Arial,sans-serif;">
        ...
        <table width="600" cellpadding="0" cellspacing="0" style="background:#1a1008;border-radius:12px;overflow:hidden;border:1px solid #3d2b10;">
          ...
          <tr>
            <td style="background:linear-gradient(135deg,#7c3a0a,#c46c1a);padding:28px 32px;text-align:center;">
              ...
              <p style="margin:8px 0 0;font-size:14px;color:#fde8c0;opacity:0.9;">
                Someone just signed up for Tater Tracker
              </p>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding:28px 32px;">
              <div style="background:#12080a;border:1px solid #3d2b10;border-radius:8px;padding:20px 24px;margin-bottom:24px;">
                <div style="font-size:11px;text-transform:uppercase;letter-spacing:1px;color:#6b5030;margin-bottom:10px;">New Member</div>
                <div style="font-size:20px;font-weight:700;color:#f5e6cc;margin-bottom:4px;">${name}</div>
                <div style="font-size:14px;color:#9a7c55;margin-bottom:8px;">${email}</div>
                <div style="font-size:12px;color:#6b5030;">Joined ${joinedAt}</div>
              </div>
              ...
              <table width="100%" cellpadding="0" cellspacing="0">
                <tr>
                  <td align="center">
                    <a href="${adminUrl}"
                       style="display:inline-block;background:linear-gradient(135deg,#c46c1a,#e8943a);color:#fff;font-weight:700;font-size:15px;text-decoration:none;padding:14px 36px;border-radius:8px;">
                      View Members →
                    </a>
                  </td>
                </tr>
          ...
          <tr>
            <td style="padding:16px 32px 24px;border-top:1px solid #2a2118;text-align:center;">
              <p style="margin:0;font-size:12px;color:#6b5030;">
                Tater Tracker · Member Notifications
              </p>
            </td>
          </tr>
  ...
  try {
    ...
    if (error) console.error('[email] Resend error (new-user):', error)
    else        console.log('[email] New-user email sent:', data?.id)
  } catch (err) {
    console.error('[email] Failed to send new-user email:', err)
  }
}
