...
function makeApproveToken(email) {
  return crypto
...
export async function sendApprovalRequestEmail(name, email) {
  ...
  const approveToken = makeApproveToken(email)
  const approveUrl   = `${appUrl}/api/admin/approve?email=${encodeURIComponent(email)}&token=${approveToken}`
    ...
    <tr>
      <td align="center">
        <table width="600" cellpadding="0" cellspacing="0" style="background:#1a1008;border-radius:12px;overflow:hidden;border:1px solid #3d2b10;">
          ...
          <!-- User info + approve button -->
          <tr>
            <td style="padding:28px 32px;">
              <div style="background:#12080a;border:1px solid #3d2b10;border-radius:8px;padding:20px 24px;margin-bottom:28px;">
              ...
              <table width="100%" cellpadding="0" cellspacing="0">
                <tr>
                  <td align="center" style="padding-bottom:20px;">
                    <a href="${approveUrl}"
                       style="display:inline-block;background:linear-gradient(135deg,#166534,#15803d);color:#fff;font-weight:800;font-size:16px;text-decoration:none;padding:16px 48px;border-radius:10px;letter-spacing:0.2px;">
                      ✅ Approve ${name}
                    </a>
                  </td>
                </tr>
              ...
              <p style="margin:0;font-size:12px;color:#6b5030;text-align:center;line-height:1.6;">
                Or manage all pending users at
                <a href="${adminUrl}" style="color:#c9a87a;">${adminUrl}</a>
              </p>
            </td>
          </tr>
...
// ── Tatera Costco alert email ─────────────────────────────────────────────────

/**
 * Send a single Costco bourbon alert relayed from Tatera's Illinois Discord.
 *
 * @param {object} alert
 * @param {string} alert.productName  — e.g. "Jack Daniel's 12 Year"
 * @param {string} alert.itemNumber   — Costco item number, e.g. "1737672"
 * @param {string} alert.storeName    — e.g. "Naperville"
 * @param {string} alert.storeNumber  — e.g. "342"
 * @param {string} alert.state        — e.g. "IL"
 * @param {'in_stock'|'out_of_stock'} alert.status
 * @param {string} alert.observedAt   — ISO timestamp from Discord message
 */
export async function sendTateraAlert(alert) {
  const to = process.env.ALERT_EMAIL
  if (!to) {
    console.warn('[email] ALERT_EMAIL not set — skipping Tatera alert email')
    return
  }
  if (alert.status !== 'in_stock') return  // only email on in-stock transitions

  const product   = escapeHtml(alert.productName)
  const storeStr  = `${escapeHtml(alert.storeName)} (${escapeHtml(alert.storeNumber)}), ${escapeHtml(alert.state)}`
  const subject   = `🥃 Costco ${alert.state}: ${alert.productName} at ${alert.storeName}`
  const finderUrl = 'https://www.costco.com/warehouse-locations'

  const observed  = new Date(alert.observedAt).toLocaleString('en-US', { dateStyle: 'medium', timeStyle: 'short' })
...
<body style="margin:0;padding:0;background:#0f0a05;color:#f5e6cc;font-family:'Segoe UI',Arial,sans-serif;">
        ...
        <table width="600" cellpadding="0" cellspacing="0" style="background:#1a1008;border-radius:12px;overflow:hidden;border:1px solid #3d2b10;">
          <tr>
            <td style="background:linear-gradient(135deg,#7c3a0a,#c46c1a);padding:28px 32px;text-align:center;">
              <div style="font-size:32px;margin-bottom:6px;">🥃</div>
              <h1 style="margin:0;font-size:22px;color:#fff;font-weight:800;letter-spacing:-0.5px;">
                Costco ${escapeHtml(alert.state)} Alert
              </h1>
              <p style="margin:6px 0 0;font-size:13px;color:#fde8c0;opacity:0.9;">
                Bottle just went in-stock at a Costco warehouse
              </p>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding:24px 32px;">
              <div style="background:#12080a;border:1px solid #3d2b10;border-radius:8px;padding:18px 20px;">
                <div style="font-size:11px;text-transform:uppercase;letter-spacing:1px;color:#6b5030;margin-bottom:6px;">Now Available</div>
                <div style="font-size:18px;font-weight:700;color:#e8943a;margin-bottom:14px;">${product}</div>

                <table width="100%" cellpadding="0" cellspacing="0" style="font-size:13px;">
                  <tr>
                    <td style="padding:6px 0;color:#6b5030;width:110px;">Store</td>
                    <td style="padding:6px 0;color:#f5e6cc;font-weight:600;">${storeStr}</td>
                  </tr>
                  <tr>
                    <td style="padding:6px 0;color:#6b5030;">Item #</td>
                    <td style="padding:6px 0;color:#c9a87a;font-family:Consolas,monospace;">${escapeHtml(alert.itemNumber)}</td>
                  </tr>
                  <tr>
                    <td style="padding:6px 0;color:#6b5030;">Spotted</td>
                    <td style="padding:6px 0;color:#9a7c55;">${observed}</td>
                  </tr>
                </table>
              </div>
            </td>
          ...
          <tr>
            <td style="padding:0 32px 20px;text-align:center;">
              <a href="${finderUrl}"
                 style="display:inline-block;background:linear-gradient(135deg,#c46c1a,#e8943a);color:#fff;font-weight:700;font-size:14px;text-decoration:none;padding:12px 28px;border-radius:8px;">
                Find this Costco →
              </a>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding:16px 32px 24px;border-top:1px solid #2a2118;text-align:center;">
              <p style="margin:0;font-size:12px;color:#6b5030;">
                Relayed from Tatera Illinois feed · Tater Tracker
              </p>
            </td>
          </tr>
        </table>
  ...
  try {
    ...
    if (error) console.error('[email] Resend error (tatera):', error)
    else        console.log('[email] Tatera alert email sent:', data?.id)
  } catch (err) {
    console.error('[email] Failed to send Tatera alert email:', err)
  }
}
