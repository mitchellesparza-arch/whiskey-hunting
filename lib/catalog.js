...
function score(query, candidate) {
  ...
  // Numeric hard-reject: age/year numbers in the query must appear in the candidate
  const qNums = q.split(' ').filter(w => /^\d+$/.test(w))
  const cNums = c.split(' ').filter(w => /^\d+$/.test(w))
