...
export function getYearVariants(baseName) {
  // Strip only a 4-digit YEAR token (1900–2099), not any 4-digit run — otherwise
  // "Old Forester 1920" loses its product-name "1920", and "Sazerac 18 Year"
  // is unaffected. Anchored to word boundaries.
  const stripYear = s => norm(s).replace(/\b(?:19|20)\d{2}\b/g, '').replace(/\s+/g, ' ').trim()
  const normBase  = stripYear(baseName)
  return data
    .filter(e => e.year && stripYear(e.name) === normBase)
