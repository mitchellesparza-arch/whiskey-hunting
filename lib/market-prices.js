...
function scoreMatch(norm, candidate) {
  // Numeric hard-reject: every number in the query (age, year, proof) must
  // appear in the candidate. Prevents "22 Year" from matching a "7 Year" entry
  // because the length->=3 filter drops short numbers like "7" and "22".
  const qNums = norm.split(/\s+/).filter(w => /^\d+$/.test(w))
  const cNums = candidate.split(/\s+/).filter(w => /^\d+$/.test(w))
