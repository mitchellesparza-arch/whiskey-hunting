...
/**
 * Save a user-chosen display name and/or Discord handle.
 * Sets customName: true so sign-in won't overwrite it.
 */
export async function updateUserProfile(email, { name, discordHandle } = {}) {
  if (!email) return
  const redis   = getRedis()
  const current = await redis.hget(USERS_KEY, email)
  const parsed  = current ? (typeof current === 'string' ? JSON.parse(current) : current) : null
