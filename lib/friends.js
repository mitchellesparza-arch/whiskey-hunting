...
export async function registerUser(email, name) {
  ...
  const entry   = {
    ...
    notifPrefs:    parsed?.notifPrefs    ?? null,  // null = all enabled (user hasn't configured yet)
  }
 ...
 */
export async function updateUserProfile(email, { name, discordHandle, muleRequests, notifPrefs, costcoFavorites, costcoMode } = {}) {
  if (!email) return
  const redis   = getRedis()
  const current = await redis.hget(USERS_KEY, email)
  const parsed  = current ? (typeof current === 'string' ? JSON.parse(current) : current) : null
