 ...
 *
 * Redis keys:
 *   wh:users                  — Hash: email → JSON { name, joinedAt, tier, ... }
 ...
 *
 * Tier fields on user record:
 *   tier               — 'free' | 'pro' | 'grandfathered'
 *   stripeCustomerId   — Stripe customer ID (null for free users)
 *   subscriptionId     — Stripe subscription ID (null for free users)
 *   subscriptionStatus — Stripe subscription status string (null for free users)
 */
...
export async function registerUser(email, name) {
  ...
  const isNew   = !parsed
  const entry   = {
    // If the user has set a custom display name in settings, keep it.
    // Otherwise fall back to the Google name from sign-in.
    name:               parsed?.customName ? parsed.name : (name ?? parsed?.name ?? email.split('@')[0]),
    joinedAt:           parsed?.joinedAt ?? new Date().toISOString(),
    discordHandle:      parsed?.discordHandle ?? null,
    customName:         parsed?.customName ?? false,
    muleRequests:       parsed?.muleRequests       ?? [],   // preserve on every sign-in
    notifPrefs:         parsed?.notifPrefs         ?? null, // null = all enabled
    // Tier / subscription — always preserved on sign-in, never reset
...
/**
 * Save a user-chosen display name, Discord handle, and/or mule requests.
 * Sets customName: true only when name is explicitly provided, so sign-in
 * won't overwrite a custom name but mule-request-only saves won't lock the name.
 */
export async function updateUserProfile(email, {
  name, discordHandle, muleRequests, notifPrefs, costcoFavorites, costcoMode,
  // Tier / subscription fields
  tier, stripeCustomerId, subscriptionId, subscriptionStatus,
} = {}) {
  if (!email) return
  const redis   = getRedis()
  const current = await redis.hget(USERS_KEY, email)
  ...
  const entry   = {
    ...
    joinedAt:           parsed?.joinedAt ?? new Date().toISOString(),
    discordHandle:      discordHandle ?? parsed?.discordHandle ?? null,
    customName:         name !== undefined ? true : (parsed?.customName ?? false),
    muleRequests:       muleRequests    !== undefined ? muleRequests    : (parsed?.muleRequests    ?? []),
    notifPrefs:         notifPrefs      !== undefined ? notifPrefs      : (parsed?.notifPrefs      ?? null),
    costcoFavorites:    costcoFavorites !== undefined ? costcoFavorites : (parsed?.costcoFavorites ?? []),
    costcoMode:         costcoMode      !== undefined ? costcoMode      : (parsed?.costcoMode      ?? 'all'),
    // Tier / subscription — preserve existing unless explicitly passed
    tier:               tier               !== undefined ? tier               : (parsed?.tier               ?? TIERS.FREE),
    stripeCustomerId:   stripeCustomerId   !== undefined ? stripeCustomerId   : (parsed?.stripeCustomerId   ?? null),
    subscriptionId:     subscriptionId     !== undefined ? subscriptionId     : (parsed?.subscriptionId     ?? null),
    subscriptionStatus: subscriptionStatus !== undefined ? subscriptionStatus : (parsed?.subscriptionStatus ?? null),
  }
  await redis.hset(USERS_KEY, { [email]: JSON.stringify(entry) })
  return entry
}
