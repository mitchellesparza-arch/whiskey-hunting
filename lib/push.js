...
// Only configure VAPID when keys are present — avoids throwing during local
// builds / data collection where env vars aren't set.
// Wrapped in try/catch so an invalid key format never crashes module load
// (which would break any route that imports lib/finds.js → lib/push.js).
...
/**
 * Send a push notification to all Pro and Grandfathered users with an active
 * push subscription.  Free-tier users are silently excluded.
 *
 * Used for high-value one-shot alerts (e.g. ReserveBar limited releases) that
 * are part of the Pro feature set.
 */
export async function sendProBroadcast(payload) {
  if (!_vapidConfigured) return
  try {
    const redis  = getRedis()
    const emails = await redis.smembers(ENABLED_SET)
    ...
    const eligible = emails.filter(email => {
      const profile = profileMap[email.toLowerCase()]
      if (!profile) return false  // no profile = unknown tier = exclude
      const tier = profile.tier ?? 'free'
      return tier === 'pro' || tier === 'grandfathered'
    })

    console.log(`[push] sendProBroadcast: ${eligible.length}/${emails.length} eligible (pro/grandfathered)`)
    await Promise.allSettled(eligible.map(e => sendToUser(e, payload)))
  } catch (err) {
    console.error('[push] sendProBroadcast error:', err)
  }
}

/**
 * Costco-specific broadcast that respects each user's costcoMode + costcoFavorites:
 *   - notifPrefs.costco === false  → skip
 *   - costcoMode === 'favorites' && storeNumber not in costcoFavorites → skip
 *   - otherwise → send
 *
 * Default behavior for users that never configured Costco prefs: notifPrefs.costco
 * defaults to enabled, costcoMode defaults to 'all' — i.e. they get every alert.
 */
export async function sendCostcoBroadcast(payload, storeNumber) {
  if (!_vapidConfigured) return
  try {
    const redis  = getRedis()
    const emails = await redis.smembers(ENABLED_SET)
    ...
    const targetStore = String(storeNumber)
    const eligible = emails.filter(email => {
      const profile = profileMap[email.toLowerCase()]
      if (!profile) return true  // never configured = enabled, mode 'all'

      // Costco notifications opt-out
      if (profile.notifPrefs?.costco === false) return false

      // Mode gate
      if (profile.costcoMode === 'favorites') {
        const favs = Array.isArray(profile.costcoFavorites) ? profile.costcoFavorites.map(String) : []
        return favs.includes(targetStore)
      }
      return true
    })
    ...
    console.error('[push] sendCostcoBroadcast error:', err)
  }
}
