...
// Search terms covering every CITY_HIVE_TARGETS entry.
// All queries run in parallel per store via search_preview.json.
const SEARCH_TERMS = [
  // Buffalo Trace / Sazerac
  'eagle rare',
  'blanton',
...
// Search-preview approach — uses /api/v1/products/search_preview.json with targeted
// queries instead of paginating the full catalog. Falls back to HTML if blocked.
export async function checkCityHiveStoreSearch(store) {
  const found   = new Map()
  ...
  await Promise.all(SEARCH_TERMS.map(async (term) => {
    try {
      const params = new URLSearchParams({
        api_key:                          CITY_HIVE_API_KEY,
        text:                             term,
        merchant_id:                      store.merchantId,
        local:                            'true',
        ch_request_flag_fast_search_preview: 'true',
      })
      const res = await fetch(`${store.base}/api/v1/products/search_preview.json?${params}`, {
        headers: { 'User-Agent': 'Mozilla/5.0' },
        cache:   'no-store',
        signal:  AbortSignal.timeout(2000),
      })
      if (!res.ok) { if (res.status === 403) blocked = true; return }
      accessible = true
      ...
      for (const product of products) {
        ...
        for (const target of CITY_HIVE_TARGETS) {
          ...
          const merchant = (product.merchants ?? []).find(m => m.merchant_id === store.merchantId)
                        ?? product.merchants?.[0]
          const option   = merchant?.product_options?.[0]
          found.set(target.name, {
            ...
            inStock:  true,
            price:    option?.price != null ? parseFloat(option.price) : null,
    ...
    } catch { /* timeout or network error — skip this term */ }
  }))

  console.log(`[CH-SEARCH] ${store.name} — blocked:${blocked} accessible:${accessible} finds:${found.size}`)

  // Fall back to HTML whenever no search succeeded (blocked, timeout, or other error)
  if (!accessible) return checkCityHiveStoreHTML(store)

  return { finds: [...found.values()], catalogSize: SEARCH_TERMS.length, accessible }
}

// Full-catalog REST API check (500 products, fast, but Cloudflare-blocked from Vercel).
// Kept as reference; replaced by checkCityHiveStoreSearch in runCityHiveChecks.
export async function checkCityHiveStoreAPI(store) {
  ...
  try {
    ...
    if (!res.ok) {
      if (process.env.CITY_HIVE_MODE === 'api') return { finds: [], catalogSize: 0, accessible: false }
    ...
    // In API mode (home machine / residential IP) accept 0 products as a valid
    // empty result — no HTML fallback. HTML pagination from home caused rate limits.
    // Only fall back to HTML on Vercel (where the REST API is blocked entirely).
    if (products.length === 0 && process.env.CITY_HIVE_MODE === 'api') {
      return { finds: [], catalogSize: 0, accessible: true }
    }
    if (products.length === 0) return checkCityHiveStoreHTML(store)
...
// HTML scraper — paginates /shop/?subtype=X pages, 5 at a time, with early stopping.
// Firing all 40 pages at once overwhelms slower store servers (Sal's, Antioch, Elgin).
// Batch-of-5 keeps concurrency at 10 per store (5 whiskey + 5 bourbon in parallel),
// stops a subtype when a full batch returns no JSON-LD (catalog exhausted).
const HTML_SUBTYPES   = ['whiskey', 'bourbon']
const HTML_MAX_PAGES  = 20    // cap: 20 × 18 = 360 products per subtype
const HTML_BATCH      = 3     // pages fetched in parallel per batch (reduced to avoid rate limits)
const HTML_BATCH_MS   = 250   // ms pause between batches

async function fetchCityHivePage(store, subtype, page) {
  try {
    // Page 1: no page param — avoids stores that 404/redirect on explicit page=1
    const url = page === 1
      ? `${store.base}/shop/?subtype=${subtype}`
      : `${store.base}/shop/?subtype=${subtype}&page=${page}`
    ...
    const block  = blocks.find(b => b[1].includes('"ItemList"'))
    if (!block) return null
    const items = JSON.parse(block[1]).itemListElement ?? []
...
export async function checkCityHiveStoreHTML(store) {
  ...
  function processItems(items) {
    ...
    for (const listItem of items) {
      ...
      const inStock = /InStock/i.test(offers?.availability ?? '')
      const price   = offers?.price != null ? parseFloat(offers.price) : null
      const url     = product.url || `${store.base}/shop/`
      for (const target of CITY_HIVE_TARGETS) {
        if (!target.pattern.test(name)) continue
        if (found.has(target.name)) continue
        found.set(target.name, {
  ...
  // Both subtypes paginate in parallel; within each subtype pages are batched
  // with a pause between batches to avoid triggering Cloudflare rate limits.
  await Promise.all(HTML_SUBTYPES.map(async (subtype) => {
    for (let start = 1; start <= HTML_MAX_PAGES; start += HTML_BATCH) {
      if (start > 1) await new Promise(r => setTimeout(r, HTML_BATCH_MS))
      const pages   = Array.from({ length: Math.min(HTML_BATCH, HTML_MAX_PAGES - start + 1) }, (_, i) => start + i)
      const results = await Promise.all(pages.map(p => fetchCityHivePage(store, subtype, p)))
      results.forEach(processItems)
      if (!results.some(Boolean)) break  // whole batch was empty — catalog exhausted
    }
  }))
 ...
 */
export { CITY_HIVE_STORES }

export async function runCityHiveChecks() {
  // When CITY_HIVE_MODE=api (home machine / residential IP) use the REST API
  // for full 500-product catalogs. On Vercel (datacenter IP, Cloudflare-blocked)
  // fall back to the HTML paginator.
  const useAPI = process.env.CITY_HIVE_MODE === 'api'
  const promises = CITY_HIVE_STORES.map(store =>
    (useAPI && store.merchantId) ? checkCityHiveStoreAPI(store) : checkCityHiveStoreHTML(store)
  )
  const results = await Promise.all(promises)
  return { results, ordered: CITY_HIVE_STORES }
}
