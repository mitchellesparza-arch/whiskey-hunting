  ...
  try {
    ...
    const res = await fetch(`${store.base}/api/v1/products/search.json?${params}`, {
      headers: {
        ...
        'Referer':         `${store.base}/shop/?subtype=whiskey`,
    ...
    if (!res.ok) {
      console.error(`[cityhive] ${store.name} API ${res.status}: ${await res.text().then(t=>t.slice(0,200)).catch(()=>'')}`)
    ...
    console.error(`[cityhive] ${store.name} fetch threw: ${e?.message ?? e}`)
    return checkCityHiveStoreHTML(store)
  }
}
