...
// Search terms — one per bottle group, covering all CITY_HIVE_TARGETS.
// Used by checkCityHiveStoreSearch: ~35 targeted queries instead of paginating
// through 1000+ products. Each query is 2-3 kB; all run in parallel per store.
const SEARCH_TERMS = [
  'eagle rare', "blanton's", 'weller special reserve', 'weller 12',
  'weller antique', 'weller full proof', 'weller cypb', 'william larue weller',
  'george t stagg', 'stagg jr', 'thomas h handy', 'sazerac 18',
  'e.h. taylor', 'rock hill farms', 'old charter oak',
  'pappy van winkle', 'van winkle', 'old rip van winkle',
  'old fitzgerald', 'elijah craig barrel proof', 'elijah craig toasted',
  "parker's heritage", 'larceny barrel proof', 'heaven hill heritage',
  'old forester birthday', 'king of kentucky', "booker's",
  'little book', "baker's 13", "knob creek 12", "knob creek 18",
  "maker's mark cellar", "russell's reserve 13", 'wild turkey master',
  "angel's envy cask", 'four roses limited', 'whistlepig 18',
  "michter's 10", 'barrell', 'willett family estate', 'peerless',
  'blood oath pact', 'blade and bow 30', 'elmer t lee',
  'high west bourye', 'woodford reserve batch proof', 'smoke wagon', 'joseph magnus',
  'weller single barrel', 'weller millennium', 'double eagle very rare',
]
...
export async function checkCityHiveStoreSearch(store) {
  ...
  // If the search endpoint was fully blocked, fall back to HTML scraping
  if (blocked && !accessible) return checkCityHiveStoreHTML(store)
...
export async function checkCityHiveStoreHTML(store) {
  ...
  for (const pagePath of (store.pages ?? [])) {
    ...
    } catch { /* network / parse error — try next page */ }
  }
...
export async function runCityHiveChecks() {
  ...
  for (const store of CITY_HIVE_STORES) {
    if (store.merchantId) {
      promises.push(checkCityHiveStoreSearch(store))
    } else {
