...
export async function checkCityHiveStoreHTML(store) {
  ...
  for (const pagePath of (store.pages ?? [])) {
    try {
      ...
      if (!res.ok) {
        console.error(`[CH-HTML] ${store.name} ${pagePath} HTTP ${res.status}`)
        continue
      }
      ...
      if (!itemListBlock) {
        console.warn(`[CH-HTML] ${store.name} ${pagePath} — no ItemList JSON-LD found`)
        continue
      }
      ...
      console.error(`[CH-HTML] ${store.name} ${pagePath} exception — ${e.message}`)
    }
  }

  console.log(`[CH-HTML] ${store.name} — accessible:${accessible} catalogSize:${catalogSize} finds:${found.size}`)
