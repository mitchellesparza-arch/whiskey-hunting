...
export async function checkCityHiveStoreAPI(store) {
  ...
  try {
    ...
    params.set('per_page', '500')
    // No subtype filter — stores tag products inconsistently ('bourbon', 'whiskey',
    // 'american whiskey', 'spirits', etc.). Filtering here misses products tagged
    // differently. CITY_HIVE_TARGETS patterns are specific enough to avoid false
    // positives from beer/wine, so we scan the full 500-product result set.
    ...
    if (!res.ok) return checkCityHiveStoreHTML(store)

    const data     = await res.json()
    const products = data.products ?? []

    // If API returns 0 products the store likely uses different field names —
    // fall back to HTML so we still get some coverage.
...
const HTML_BATCH     = 5    // pages fetched in parallel within each batch
