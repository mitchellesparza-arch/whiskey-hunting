...
// HTML scraper — fetches up to MAX_PAGES pages per subtype in parallel.
// City Hive SSR embeds 18 products per page as JSON-LD; paginating covers the
// full catalog without needing the Cloudflare-blocked REST API.
const HTML_SUBTYPES  = ['whiskey', 'bourbon']
const HTML_MAX_PAGES = 20   // 20 × 18 = 360 products per subtype

async function fetchCityHivePage(store, subtype, page) {
  try {
    // Page 1: no page param — base URL works on all stores; page=1 causes issues on some
    const url = page === 1
...
export async function checkCityHiveStoreHTML(store) {
  ...
  // Fetch all pages for all subtypes in parallel
  const tasks = HTML_SUBTYPES.flatMap(subtype =>
    Array.from({ length: HTML_MAX_PAGES }, (_, i) => ({ subtype, page: i + 1 }))
  )

  const results = await Promise.all(tasks.map(({ subtype, page }) =>
    fetchCityHivePage(store, subtype, page)
  ))
