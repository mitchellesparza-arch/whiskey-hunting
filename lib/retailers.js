...
const CITY_HIVE_STORES = [
  // ── REST API stores (merchantId known) ────────────────────────────────────
  ...
  // ── HTML scrape fallback (merchantId not yet known) ───────────────────────
  // Sal's: 3 separate City Hive subdomains
...
async function checkCityHiveStoreAPI(store) {
  const found = new Map()
  try {
    const params = new URLSearchParams()
    ...
    if (!res.ok) return checkCityHiveStoreHTML(store) // fall back on API failure
...
async function checkCityHiveStoreHTML(store) {
  const found       = new Map()
  let   catalogSize = 0
...
async function checkAllCityHiveStores() {
  const results = await Promise.all(
    CITY_HIVE_STORES.map(store => store.merchantId
      ? checkCityHiveStoreAPI(store)
      : checkCityHiveStoreHTML(store)
    )
  )
