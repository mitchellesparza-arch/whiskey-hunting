...
//
// All queries route through malloysfinest.com regardless of the store's own domain.
// The merchant_id still scopes results to the correct store — the base URL is just
// a proxy to the shared City Hive backend. Individual store domains vary in their
// Cloudflare aggressiveness; malloysfinest.com is confirmed to pass server-side.
const CITY_HIVE_PROXY_BASE = 'https://malloysfinest.com'
...
export async function checkCityHiveStoreAPI(store) {
  ...
  try {
    ...
    const res = await fetch(`${CITY_HIVE_PROXY_BASE}/api/v1/products/search.json?${params}`, {
      headers: {
        'User-Agent':      'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36',
        'Accept':          'application/json, text/plain, */*',
        'Accept-Language': 'en-US,en;q=0.9',
        'Referer':         `${CITY_HIVE_PROXY_BASE}/shop/?subtype=whiskey`,
