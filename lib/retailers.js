...
function parseLiquorBarnPickup(html, bottleName, price, url) {
  const results = []
  ...
  for (const item of items) {
    ...
    if (!loc) continue
    results.push({ bottle: bottleName, retailer: loc.retailer, location: loc.location, inStock, price: inStock ? price : null, url })
  }
  return results
}
...
async function checkLiquorBarn() {
  ...
  await Promise.all(LIQUOR_BARN_BOTTLES.map(async (bottle) => {
    try {
      ...
      // Step 2: per-location pickup availability via Shopify Section Rendering API
      if (variantId) {
        try {
          ...
          if (pickupRes.ok) {
            ...
            const locationFinds = parseLiquorBarnPickup(html, bottle.name, price, url)
            if (locationFinds.length > 0) {
              results.push(...locationFinds)
              return
            }
          }
        } catch { /* fall through to single-entry fallback */ }
      }

      // Fallback: globally available but Section Rendering API blocked or empty
      results.push({
        bottle:   bottle.name,
...
const CITY_HIVE_STORES = [
  // ── Original stores ───────────────────────────────────────────────────────
  { name: "Joe's Beverage Warehouse",    location: 'Romeoville, IL',                    base: 'https://joesbev.com',                    pages: ['/shop/?subtype=whiskey&per_page=500', '/shop/?subtype=bourbon&per_page=500'] },
  { name: 'Uncork It Chicago',           location: 'Chicago, IL',                        base: 'https://uncorkitchicago.com',            pages: ['/shop/?subtype=whiskey&per_page=500', '/shop/?subtype=bourbon&per_page=500'] },
  // Kenwood Liquors: /shop/ returns HTTP 404 — removed until resolved

  // ── South / Southwest suburbs ─────────────────────────────────────────────
  { name: "John's Beverage Warehouse",   location: 'Joliet, IL',                         base: 'https://johnsbev.com',                   pages: ['/shop/?subtype=whiskey&per_page=500', '/shop/?subtype=bourbon&per_page=500'] },
  { name: 'Greene Valley Wine & Spirits', location: 'Bolingbrook, IL',                   base: 'https://greenevalleyws.com',             pages: ['/shop/?subtype=whiskey&per_page=500', '/shop/?subtype=bourbon&per_page=500'] },
  // Sal's: 3 separate City Hive subdomains — check all for maximum coverage
  { name: "Sal's Beverage World (Elmhurst)",  location: 'Elmhurst, IL',   base: 'https://elmhurst.salsbeverageworld.com',  pages: ['/shop/?subtype=whiskey&per_page=500', '/shop/?subtype=bourbon&per_page=500'] },
  { name: "Sal's Beverage World (Addison)",   location: 'Addison, IL',    base: 'https://addison.salsbeverageworld.com',   pages: ['/shop/?subtype=whiskey&per_page=500', '/shop/?subtype=bourbon&per_page=500'] },
  { name: "Sal's Beverage World (Villa Park)", location: 'Villa Park, IL', base: 'https://villapark.salsbeverageworld.com', pages: ['/shop/?subtype=whiskey&per_page=500', '/shop/?subtype=bourbon&per_page=500'] },

  // ── Chicago city ──────────────────────────────────────────────────────────
  { name: 'Archer Liquors',              location: 'Chicago (SW), IL',                  base: 'https://archerliquors.com',              pages: ['/shop/?subtype=whiskey&per_page=500', '/shop/?subtype=bourbon&per_page=500'] },
  { name: 'Liquor Expo Chicago',         location: 'Chicago (Lincoln Park), IL',        base: 'https://myliquorexpo.com',               pages: ['/shop/?subtype=whiskey&per_page=500', '/shop/?subtype=bourbon&per_page=500'] },

  // ── North / Northwest suburbs ─────────────────────────────────────────────
  { name: 'Northshore Wine & Spirits',   location: 'Highwood, IL',                      base: 'https://northshorewineandspirit.com',    pages: ['/shop/?subtype=whiskey&per_page=500', '/shop/?subtype=bourbon&per_page=500'] },
  { name: 'Gold Eagle Wine & Spirits',   location: 'Libertyville, IL',                  base: 'https://goldeaglewine.com',              pages: ['/shop/?subtype=whiskey&per_page=500', '/shop/?subtype=bourbon&per_page=500'] },
  { name: 'D&D Smoke & Spirits',         location: 'Arlington Heights, IL',             base: 'https://ddsmokespirits.com',             pages: ['/shop/?subtype=whiskey&per_page=500', '/shop/?subtype=bourbon&per_page=500'] },
  { name: 'Elgin Liquor & Wine',         location: 'Elgin, IL',                         base: 'https://elginliquor.com',                pages: ['/shop/?subtype=whiskey&per_page=500', '/shop/?subtype=bourbon&per_page=500'] },

  // ── New additions — confirmed allocated inventory ─────────────────────────
  { name: 'Delaware Food & Liquor',      location: 'Chicago (Gold Coast), IL',           base: 'https://delawarefoodandliquor.com',      pages: ['/shop/?subtype=whiskey&per_page=500', '/shop/?subtype=bourbon&per_page=500'] },
  { name: 'Antioch Fine Wine & Liquors', location: 'Antioch, IL',                        base: 'https://antiochwine.com',               pages: ['/shop/?subtype=whiskey&per_page=500', '/shop/?subtype=bourbon&per_page=500'] },
  { name: 'Armanetti Express',           location: 'Algonquin, IL',                      base: 'https://armanettiexpress.com',          pages: ['/shop/?subtype=whiskey&per_page=500', '/shop/?subtype=bourbon&per_page=500'] },
  { name: 'Armanetti Woodstock',         location: 'Woodstock, IL',                      base: 'https://armanettiwoodstock.com',        pages: ['/shop/?subtype=whiskey&per_page=500', '/shop/?subtype=bourbon&per_page=500'] },
]
...
export const RETAILERS = [
  ...
  { name: 'Liquor Barn (Niles)',        location: 'Niles, IL',        url: 'https://theliquorbarn.com', lat: 42.0167, lng: -87.8250 },
