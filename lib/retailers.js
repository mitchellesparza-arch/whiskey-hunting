...
async function checkLiquorBarn() {
  ...
  await Promise.all(LIQUOR_BARN_BOTTLES.map(async (bottle) => {
    try {
      // Fetch the product page HTML — it embeds both available AND inventory_quantity
      // (the .js/.json endpoints hide qty; the HTML includes it in the page JSON)
      const productUrl = `${LIQUOR_BARN_BASE}/products/${bottle.slug}`
      ...
      // Extract available and inventory_quantity from the embedded variant JSON
      const available = html.match(/"available"\s*:\s*(true|false)/)?.[1] === 'true'
      const qtyMatch  = html.match(/"inventory_quantity"\s*:\s*(-?\d+)/)
      const qty       = qtyMatch ? parseInt(qtyMatch[1]) : null // null = hidden by store

      // Extract price from the variants JSON array embedded in the page.
      // Shopify uses integer cents in the variants array: {"price":6999,...}
      // (The first "price" match in the page hits a currency object — skip it)
      const variantsJson = html.match(/"variants"\s*:\s*(\[[\s\S]*?\])/)?.[1]
      const variantPrice = variantsJson?.match(/"price"\s*:\s*(\d+)/)?.[1]
      const price = variantPrice ? parseInt(variantPrice) / 100 : null
      ...
      results.push({
        ...
        inStock:  available,
        price,
        // qty = aggregate across all 3 Liquor Barn locations (Wheeling/Niles/Vernon Hills)
        // null means they've hidden the count (usually secondary-market priced items)
        quantity: available ? qty : 0,
        url:      productUrl,
        slug:     bottle.slug,
      })
    } catch {
...
export async function checkAllRetailers() {
  ...
  const diagnostics = {
    ...
    ...Object.fromEntries(MALLOYS_LOCATIONS.map(l => [l.name, { accessible: true, catalogSize: null, source: 'cityhive-api' }])),
    ...Object.fromEntries(
