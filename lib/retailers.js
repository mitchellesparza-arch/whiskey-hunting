...
export async function checkCityHiveStoreAPI(store) {
  const found = new Map()
  try {
    // Route through malloysfinest.com — individual store domains are all 403'd by
    // Cloudflare at the network level from Vercel IPs. malloysfinest.com is the
    // same City Hive backend but with permissive Cloudflare config.
    // Use merchant_ids[] (array) + local=true — the exact shape Malloy's uses.
    const params = new URLSearchParams()
    params.set('api_key', CITY_HIVE_API_KEY)
    params.append('merchant_ids[]', store.merchantId)
    params.set('local', 'true')
    params.set('per_page', '500')
    params.append('additional_properties[subtype][]', 'bourbon')
    params.append('additional_properties[subtype][]', 'whiskey')

    const res = await fetch(`https://malloysfinest.com/api/v1/products/search.json?${params}`, {
      headers: { 'User-Agent': 'Mozilla/5.0' },
      cache: 'no-store',
    ...
    for (const product of products) {
      ...
      for (const target of CITY_HIVE_TARGETS) {
        ...
        // With merchant_ids[] the response has a merchants[] array — find ours by ID
