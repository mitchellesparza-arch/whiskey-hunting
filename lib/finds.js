...
export async function getFinds() {
  try {
    ...
    console.log('[finds] zrange count:', members.length, 'types:', [...new Set(members.map(m => typeof m))].join(','))

    const parsed = members
      .map(m => {
        const decoded = decodeMember(m)
        if (!decoded) {
          console.warn('[finds] decodeMember null — type:', typeof m, 'preview:', JSON.stringify(m)?.slice(0, 120))
        } else {
          console.log('[finds] decoded ok:', decoded.id, decoded.submitterName, decoded.bottleName?.slice(0, 30))
        }
        return decoded
      })
      .filter(m => {
        ...
        if (m.timestamp < cutoff) {
          console.warn('[finds] filtered by cutoff:', m.id, m.submitterName, 'ts:', m.timestamp, 'cutoff:', cutoff)
          return false
        }
        return true
      })
