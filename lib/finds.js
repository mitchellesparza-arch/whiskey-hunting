...
export async function addFind({ bottleName, upc, store, photoUrl, notes, price, submittedBy, submitterName }) {
  ...
  const key = lbKey(new Date(entry.timestamp))
  await redis.hincrby(key, entry.submitterName, 1)
  await redis.expire(key, 60 * 24 * 60 * 60) // keep for 60 days then auto-clean
