...
export async function addFind({ bottleName, upc, store, photoUrl, notes, price, submittedBy, submitterName, tier }) {
  ...
  if (proUser) {
    // Pro users: fire watchlist + wishlist notifications immediately
    checkWatchlistsForFind(redis, { ...entry, photoUrl: storedPhoto })
    checkWishlistsForFind(redis, { ...entry, photoUrl: storedPhoto })
  } else {
    // Free users: queue notifications for when the find becomes visible (1 hr)
    queueDelayedNotifs(redis, { ...entry, photoUrl: storedPhoto }).catch(() => {})
  }
