...
/**
 * Returns all finds within the 7-day retention window that are currently
 * visible (visibleAt <= now), newest first.
 *
 * Each find includes a computed `status` field:
 *   'active'   — timestamp < 24 h ago
 *   'archived' — timestamp 24 h – 7 days ago
 *
 * Finds submitted by free-tier users have visibleAt = timestamp + 1 hour.
 * They are silently excluded from this response until they become visible.
  ...
  try {
    ...
    const parsed = members
      ...
      .filter(m => {
        ...
        // visibleAt defaults to timestamp for legacy entries (no visibleAt field)
        const visibleAt = m.visibleAt ?? m.timestamp
...
export async function addFind({ bottleName, upc, store, photoUrl, notes, price, submittedBy, submitterName, tier }) {
  ...
  // Free users: find is hidden for 1 hour; pro users: visible immediately
  const visibleAt = proUser ? now : now + FREE_DELAY_MS
