...
import { postFindAlert, postJonFind } from './discord.js'
import { sendToUser }     from './push.js'
import { sendBroadcast }  from './push.js'
import { getWishlist }    from './wishlist.js'
import { deleteBlobPhotos } from './blob-cleanup.js'

const KEY             = 'wh:finds'
const PHOTOS_KEY      = 'wh:find-photos'
const PENDING_KEY     = 'wh:pending-notifs'    // sorted set: score=visibleAt, member=base64url(payload)
const ACTIVE_MS       =      24 * 60 * 60 * 1000   // 24 h  — active window
const TTL_MS          = 7 * 24 * 60 * 60 * 1000    // 7 days — archive retention

const CHECKIN_KEY     = 'wh:checkins'              // sorted set: score=timestamp, member=base64url(entry)
const CHECKIN_TTL_MS  =      24 * 60 * 60 * 1000   // 24 h — "last checked" signal goes stale fast
...
function encodeMember(obj) {
  // base64url: A-Za-z0-9-_ only — no +, /, or = that Upstash REST rejects in member strings
  return Buffer.from(JSON.stringify(obj)).toString('base64url')
}

function decodeMember(m) {
  if (typeof m !== 'string') return m
  // base64url (current encoding)
  try {
    const obj = JSON.parse(Buffer.from(m, 'base64url').toString('utf-8'))
    if (obj && typeof obj === 'object') return obj
  } catch {}
  // standard base64 (one-release transition period)
  try {
    const obj = JSON.parse(Buffer.from(m, 'base64').toString('utf-8'))
    if (obj && typeof obj === 'object') return obj
  } catch {}
  // legacy raw-JSON members written before encoding was introduced
...
function checkinCutoffMs() { return Date.now() - CHECKIN_TTL_MS }

/**
 * Purges finds older than the 7-day TTL. Unlike a plain zremrangebyscore,
 * this also deletes each expired find's Blob photo (and its PHOTOS_KEY hash
 * entry) — otherwise the sorted-set pointer disappears but the actual
 * uploaded image is orphaned in Blob storage forever.
 */
async function purgeExpiredFinds(redis) {
  try {
    const cutoff   = cutoffMs()
    const expiring = await redis.zrangebyscore(KEY, 0, cutoff)

    if (expiring.length) {
      const ids = expiring.map(m => decodeMember(m)?.id).filter(Boolean)
      if (ids.length) {
        let photosMap = {}
        try { photosMap = (await redis.hgetall(PHOTOS_KEY)) ?? {} } catch {}
        const urls = ids.map(id => photosMap[id]).filter(Boolean)
        if (urls.length) await deleteBlobPhotos(urls)
        await Promise.all(ids.map(id => redis.hdel(PHOTOS_KEY, id).catch(() => {})))
      }
    }

    await redis.zremrangebyscore(KEY, 0, cutoff)
  } catch (err) {
    console.warn('[finds] purgeExpiredFinds error:', err?.message ?? err)
  }
}

/**
 * Returns finds within the 7-day retention window, newest first.
 *
 * Each find includes a computed `status` field:
 *   'active'   — timestamp < 24 h ago
 *   'archived' — timestamp 24 h – 7 days ago
 *
 * All finds are immediately visible to all users — no tier-based read delay.
 *
...
export async function getFinds() {
  try {
    ...
    const parsed = members
      ...
      .filter(m => {
        if (!m || m.timestamp < cutoff) return false
        return true
      })
...
/**
 * Returns store check-ins ("stopped by, nothing notable") within the 24 h
 * window, newest first. This is a separate, lighter-weight event from a
 * find — it only requires a store, no bottle.
 */
export async function getCheckins() {
  try {
    const redis   = getRedis()
    const cutoff  = checkinCutoffMs()
    const members = await redis.zrange(CHECKIN_KEY, 0, -1, { rev: true })
    return members
      .map(m => decodeMember(m))
...
/**
 * Adds a store check-in to the sorted set and purges entries older than 24 h.
 */
export async function addCheckin({ store, submittedBy, submitterName }) {
  const redis = getRedis()
  const now   = Date.now()
  ...
  await redis.zadd(CHECKIN_KEY, { score: now, member: encodeMember(entry) })
  await redis.zremrangebyscore(CHECKIN_KEY, 0, checkinCutoffMs()).catch(() => {})
...
/**
 * Stores a find's notification payload in the wh:pending-notifs sorted set
 * with score = visibleAt, so the cron job can send them at the right time.
 */
async function queueDelayedNotifs(redis, find) {
  const payload = {
    id:            find.id,
    bottleName:    find.bottleName,
    submitterName: find.submitterName,
    submittedBy:   find.submittedBy,
    store:         find.store,
    photoUrl:      find.photoUrl ?? null,
    notes:         find.notes    ?? null,
    price:         find.price    ?? null,
    upc:           find.upc      ?? null,
    votes:         find.votes,
    timestamp:     find.timestamp,
    visibleAt:     find.visibleAt,
  }
  await redis.zadd(PENDING_KEY, { score: find.visibleAt, member: encodeMember(payload) })
}

/**
 * Called by the cron job — processes all pending notifications whose visibleAt
 * has passed. Fires watchlist matching, wishlist matching, and the broadcast push
 * for each find, then removes it from the queue.
 *
 * Returns the number of notifications processed.
 */
export async function processPendingNotifs() {
  const redis   = getRedis()
  const now     = Date.now()

  // Purge entries older than 7 days (abandoned notifications) before processing
  await redis.zremrangebyscore(PENDING_KEY, 0, now - 7 * 24 * 60 * 60 * 1000).catch(() => {})

  // Get all due notifications (score <= now)
  const members = await redis.zrangebyscore(PENDING_KEY, 0, now)
  if (!members || members.length === 0) return 0

  let processed = 0
  for (const m of members) {
    try {
      const find = decodeMember(m)
      if (!find) continue

      // Fire all three notification channels
      checkWatchlistsForFind(redis, find).catch(() => {})
      checkWishlistsForFind(redis,  find).catch(() => {})
      sendBroadcast({
        title: '📍 New Find',
        body:  `${find.submitterName} spotted ${find.bottleName}${find.store?.name ? ` at ${find.store.name}` : ''}`,
        url:   '/',
        tag:   'find',
      }, 'finds').catch(() => {})

      // Also post to Discord (fire-and-forget)
      postJonFind(find).catch(() => {})

      await redis.zrem(PENDING_KEY, m)
      processed++
    } catch (err) {
      console.warn('[finds] processPendingNotifs error for member:', err.message)
    }
  }
...
export async function addFind({ bottleName, upc, store, photoUrl, notes, price, submittedBy, submitterName }) {
  ...
  try {
    ...
    // Purge entries older than 7 days (and their orphaned Blob photos)
    await purgeExpiredFinds(redis)
  } catch (err) {
    console.warn('[finds] post-zadd ops error (find was saved ok):', err?.message ?? err)
  }

  // All finds are now visible immediately — fire watchlist + wishlist notifications for everyone
...
export async function removeFind(id) {
  ...
  let photoUrl = null
  try { photoUrl = await redis.hget(PHOTOS_KEY, id) } catch {}
  try { await redis.hdel(PHOTOS_KEY, id) } catch {}
  if (photoUrl) await deleteBlobPhotos(photoUrl)
  return getFinds('pro')
}
...
export async function voteFind(id, type, userEmail) {
  ...
  for (const m of members) {
    try {
      ...
      await redis.zrem(KEY, m)
      await redis.zadd(KEY, { score: entry.timestamp, member: encodeMember(updated) })
