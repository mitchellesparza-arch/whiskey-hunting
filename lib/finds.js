...
const FREE_DELAY_MS   =       1 * 60 * 60 * 1000   // 1 hour delay for free-tier finds
 ...
 *
 * viewerTier controls what the requesting user can see:
 *   - Pro / grandfathered: all finds immediately
 *   - Free (default): only finds older than FREE_DELAY_MS (1 hour)
 *
 * All finds are stored with visibleAt = timestamp (immediate) — the delay
 * is applied on the read side based on the viewer's tier.
 *
 * photoUrls are merged from wh:find-photos (with legacy embedded-field fallback).
 */
export async function getFinds(viewerTier = 'free') {
  try {
    const redis     = getRedis()
    const cutoff    = cutoffMs()
    const active    = activeCutoff()
    const now       = Date.now()
    const proViewer = isPro(viewerTier)
    ...
    const parsed = members
      ...
      .filter(m => {
        ...
        // Free viewers only see finds that are at least 1 hour old
        if (!proViewer && now - m.timestamp < FREE_DELAY_MS) return false
        return true
      })
 ...
 */
export async function addFind({ bottleName, upc, store, photoUrl, notes, price, submittedBy, submitterName, tier }) {
  const redis     = getRedis()
  ...
  // All finds are stored immediately — the 1-hour delay for free users is
  // applied on the read side in getFinds() based on the viewer's tier.
  const visibleAt = now
