...
const ALGOLIA_INDEX   = 'Products_Production'
...
async function fetchCanary(bottle) {
  try {
    ...
    // Persist Algolia metadata into the canonical bottle DB.
    // Store 47 = Orland Park; we capture its live price + stock as the primary signal.
    const store47   = hit.storesPriceAndInventory?.find(s => s.storeCode === '47')
    const price47   = store47?.prices?.bestPrice ?? hit.prices?.bestPrice ?? null
    const inStock47 = (hit.inStockStores ?? []).includes('47')
    upsertBottle({
      name:            bottle.name,
      algoliaObjectId: bottle.objectID,
      binnysUrl:       bottle.url ?? null,
      binnysPrice:     price47 ? Number(price47) : null,
      binnysInStock:   inStock47,
      lastSeenBinnys:  inStock47 ? Date.now() : undefined,
      imageUrl:        hit.imageUrl ?? hit.image ?? hit.thumbnailUrl ?? null,
      nameAliases:     hit.productName && hit.productName !== bottle.name ? [hit.productName] : [],
    }, 'algolia').catch(() => {})
  ...
  const results = await Promise.all(
    canaryBottles.map(async (bottle) => {
      ...
      for (const code of storeCodes) {
        ...
        // purchaseAvailability is a string enum ("available" / "unavailable"), not
        // a stock count. The numeric quantity field name needs to be confirmed from
        // a live Algolia hit before truck-restock detection can work.
        // TODO: replace null with the correct numeric field once confirmed.
        const quantity = null
