 ...
 * Pass usersHash to enrich with display names.
 */
export async function getMuleLeaderboard() {
  try {
    ...
    // Enrich with display names from wh:users hash
    const emails  = Object.keys(scores)
    const entries = []

    for (const email of emails) {
      const raw  = await redis.hget('wh:users', email)
