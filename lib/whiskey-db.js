...
import { numbersAgree, brandAgrees } from './bottle-match.js'
import catalogData from './market-prices-data.json'
...
export const UPC_MAP = {
  ...
  '089319123679': { name: "Pappy Van Winkle 10 Year",               imageUrl: null },
...
// ── Unified seed pool ───────────────────────────────────────────────────────
// The curated catalog (market-prices-data.json, 400+ entries) is the single
// source of truth for bottle metadata. RAW_SEED only supplies bottles the
// catalog doesn't already have, so a bottle present in both can never show two
// different MSRPs (the old fragmentation: Eagle Rare was $40 here vs $35 there).
const _catalogSeed = catalogData.map(e => ({
  name:       e.name,
...
const _catalogNorms = new Set(catalogData.map(e => normalize(e.name)))
export const SEED_BOTTLES = [
  ..._catalogSeed,
  ...RAW_SEED.filter(b => !_catalogNorms.has(normalize(b.name))),
]

// Score one candidate name against the query. Numbers must agree for any tier;
// the word-overlap tier also requires the brand to agree.
function scoreOne(query, candidateName) {
  const q = normalize(query)
  const n = normalize(candidateName)
  if (n === q)              return 100
  if (!numbersAgree(query, candidateName)) return 0
  if (n.startsWith(q))     return 80    // prefix typing: "wel" → "Weller…"
  if (q.startsWith(n))     return 75
  if (n.includes(q))       return 60
  if (q.includes(n))       return 55
  if (!brandAgrees(query, candidateName)) return 0
...
// Best score across the bottle's name and any aliases (catalog entries carry
// aliases like "jd single barrel" that should also resolve).
function matchScore(query, bottle) {
  let best = 0
  for (const nm of [bottle.name, ...(bottle.aliases ?? [])]) {
    const s = scoreOne(query, nm)
    if (s > best) best = s
  }
  return best
}
...
async function setCachedUpc(upc, data, ttlSec = null) {
  try {
    if (ttlSec) await getRedis().set(`wh:upc:${upc}`, JSON.stringify(data), { ex: ttlSec })
    else        await getRedis().set(`wh:upc:${upc}`, JSON.stringify(data))
  } catch {}
}
...
export async function getBottleByUpc(upc) {
  ...
  if (productName) {
    ...
    const result = seedMatch
      ...
      // Don't fabricate a category — a raw upcitemdb title could be rye, scotch,
      // anything. Leave it null rather than mislabeling everything "Bourbon".
      : { name: productName, upc: clean, distillery: null, category: null, proof: null, msrp: null }
    // Auto-derived guesses expire (90d) so a bad/loose match can self-heal or be
    // replaced by a later user-confirmed mapping; saveUpcMapping stays permanent.
    await setCachedUpc(clean, result, 90 * 24 * 60 * 60)
