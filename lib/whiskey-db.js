...
function matchScore(query, bottle) {
  ...
  if (n === q)              return 100
  // Numbers must agree for ANY match — stops "Eagle Rare 17 Year" / "Redbreast
  // 15 Year" binding to the seed's 10/12-year entry (and inheriting its proof &
  // MSRP). Applied before the prefix tiers, which otherwise leak across ages.
  if (!numbersAgree(query, bottle.name)) return 0
  if (n.startsWith(q))     return 80    // prefix typing: "wel" → "Weller…"
  if (q.startsWith(n))     return 75
  if (n.includes(q))       return 60
  if (q.includes(n))       return 55
  // Word-overlap tier also needs the brand to agree, else a 2-word generic
  // overlap (e.g. shared "single barrel") binds a different brand's bottle.
  if (!brandAgrees(query, bottle.name)) return 0
