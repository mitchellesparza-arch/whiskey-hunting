...
/**
 * postJonFind — posts a user-reported find to Jon's Discord as a plain
 * embed message. Uses JON_FINDS_WEBHOOK, replacing the old DISCORD_FINDS_WEBHOOK post.
 *
 * NOTE: unlike postBoBFind, this does NOT send thread_name — Jon's
 * #chicagoland-hunting is a regular text channel (not a Forum channel), so
 * members manually turn a message into a thread themselves after the fact.
 * Sending thread_name here is invalid for a non-forum channel and silently
 * dropped the post entirely (found 2026-07-02).
 *
 * @param {object} find — the find entry from lib/finds.js
 */
export async function postJonFind(find) {
  const webhookUrl = process.env.JON_FINDS_WEBHOOK?.trim()
  if (!webhookUrl) {
    console.warn('[discord] JON_FINDS_WEBHOOK not configured — skipping')
    return
  }
  ...
  const ok = await postWebhook(webhookUrl, payload, `Jon find ${find.bottleName}`)
  if (ok) console.log(`[discord] Jon find posted: ${find.bottleName}`)
}
 ...
 *
 * Note: DISCORD_FINDS_WEBHOOK is kept alive solely for this function — the
 * community-finds post it used to also share now lives on postJonFind/JON_FINDS_WEBHOOK.
 */
...
export async function postBoBFind(find) {
  ...
  const rawThread  = `🥃 ${find.bottleName} — ${find.store?.name ?? 'Unknown Store'}`
  const threadName = rawThread.length > 100 ? rawThread.slice(0, 97) + '…' : rawThread
