...
export async function postNewFind(find) {
  ...
  try {
    ...
    console.log(`[discord] Find posted: ${find.bottleName}`)
  } catch (err) {
    console.warn('[discord] Find post failed:', err.message)
  }
}
...
export async function postTateraAlert(alert) {
  ...
  const payload = {
    ...
    embeds: [{
      ...
      footer:    { text: 'Relayed from Tatera Illinois feed' },
  ...
  try {
    ...
    console.log(`[discord] Tatera alert relayed: ${alert.productName} @ ${storeStr}`)
  } catch (err) {
    console.warn('[discord] Tatera alert failed:', err.message)
  }
}
