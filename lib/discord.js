...
export async function postBoBFind(find) {
  ...
  let webhookUrl
  if (testMode) {
    webhookUrl = process.env.BOB_TEST_WEBHOOK
  } else {
    const address  = find.store?.address ?? ''
    const isCity   = /\bChicago,\s*IL\b/i.test(address)
    webhookUrl     = isCity
      ? process.env.BOB_CHICAGO_WEBHOOK
      : process.env.BOB_SUBURBS_WEBHOOK
  }
  ...
  const payload = {
    ...
    embeds: [{
      ...
      url:         'https://whiskey-hunter.vercel.app/finds',
      description: `**${find.submitterName ?? 'Someone'}** just spotted this — tap the title above to see the full feed.\n\n📲 **[Log your own find on Tater Tracker](https://whiskey-hunter.vercel.app)**`,
      color:       0xe8943a,
      fields,
      image:       find.photoUrl ? { url: find.photoUrl } : undefined,
