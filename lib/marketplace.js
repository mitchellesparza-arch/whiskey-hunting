...
// Sorted-set members are base64url-encoded JSON so special chars in emails,
// zip codes, and Discord handles never trigger Upstash REST validation errors.
...
function decodeMember(m) {
  try {
    return JSON.parse(Buffer.from(m, 'base64url').toString('utf-8'))
  } catch {
    // Legacy members stored as raw JSON
...
/**
 * Purges listings older than the 60-day TTL. Also deletes each expired
 * listing's Blob photos (and its PHOTOS_KEY hash entry) — a plain
 * zremrangebyscore would drop the sorted-set pointer but leave the actual
 * uploaded images orphaned in Blob storage forever.
 */
async function purgeExpiredListings(redis) {
  try {
    ...
    console.warn('[marketplace] purgeExpiredListings error:', err?.message ?? err)
  }
}
...
export async function getListings({ type, activeOnly = false } = {}) {
  try {
    ...
    const parsed = members
      .map(m => { try { return decodeMember(m) } catch { return null } })
...
export async function addListing({
  ...
  try {
    ...
    // purge stale entries (and their orphaned Blob photos)
    await purgeExpiredListings(redis)
  } catch (err) {
...
export async function deleteListing(id) {
  ...
  if (photos.length) await deleteBlobPhotos(photos)
}
